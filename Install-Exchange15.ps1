...
<#
    ...
    - Supported Operating Systems
      ...
      - Windows Server 2022 (Desktop or Core, Exchange 2019 only)
      - Windows Server 2025 (Desktop or Core, Exchange 2019 CU15+ only)
    ...
    .REVISIONS
    See CHANGELOG.md in the repository root for the full revision history.
...
process {
    ...
    function Test-LocalCredential {
        ...
        if (!($UserName) -or ($Password.Length -eq 0)) {
            ...
            try {
                $DS.ValidateCredentials($UserName, [Runtime.InteropServices.Marshal]::PtrToStringAuto($ptr))
            }
            finally {
    ...
    function Test-Credentials {
        ...
        try {
            if ( $State['InstallEdge']) {
                ...
                return $( Test-LocalCredential -UserName $Username -Password $SecurePassword)
            }
            else {
                $ptr = [Runtime.InteropServices.Marshal]::SecureStringToBSTR($SecurePassword)
                try {
                    $dc = New-Object DirectoryServices.DirectoryEntry( $Null, $FullPlainTextAccount, [Runtime.InteropServices.Marshal]::PtrToStringAuto($ptr))
                }
                finally {
                    [Runtime.InteropServices.Marshal]::ZeroFreeBSTR($ptr)
                }
    ...
    function Test-MyPackage( $PackageID) {
        ...
        foreach ( $ID in $PackageSet) {
            ...
            $PresenceKey = (Get-CimInstance -ClassName Win32_QuickFixEngineering | Where-Object { $_.HotfixID -eq $ID }).HotfixID
            if ( !( $PresenceKey)) {
    ...
    function Set-Pagefile {
        ...
        if ($CS.AutomaticManagedPagefile) {
            ...
            try {
                ...
                Set-CimInstance -InputObject $CPF -Property @{InitialSize = $DesiredSize; MaximumSize = $DesiredSize }
            }
            catch {
    ...
    $MajorOSVersion = [string]('{0}.{1}' -f $OSVersion.Split('.')[0], $OSVersion.Split('.')[1])
        ...
        switch ($State["InstallPhase"]) {
            ...
            5 {
                ...
                if ( $State["IncludeFixes"]) {
                    ...
                    $ImagePathVersion = Get-DetectedFileVersion ( (Get-CimInstance -ClassName Win32_Service -Filter 'Name="MSExchangeServiceHost"').PathName.Trim('"') )
